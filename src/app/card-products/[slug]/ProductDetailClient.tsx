...
import { Filter, ArrowLeft, X, AlertCircle } from 'lucide-react';
...
export const ProductDetailClient: React.FC<ProductDetailClientProps> = ({ slug, initialProduct, initialDetails }) => {
  const [buyModalItem, setBuyModalItem] = useState<ProductDetail | null>(null);
  ...
  const handleBuyClick = (item: ProductDetail) => {
    ...
    <>
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="mb-10 flex flex-col md:flex-row items-start gap-4 md:gap-6">
          <Link 
            href="/card-products"
            className="group flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-white shadow-sm transition-all hover:shadow-md border border-emerald-500 text-emerald-600 hover:bg-emerald-500 hover:text-white"
          >
              ...
              <button 
                onClick={() => {
                  ...
                  if (buyModalItem.url) {
                    ...
                    alert(`正在为您跳转至渠道：${buyModalItem.channel}...`);
                  }
